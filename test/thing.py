...
def solve_triangle(values):
    ...
    def finish_angle_solve():
        ...
        for var in angle_names:
            if var not in solved_angles:
                ans = abs(solved_angles.get(key_list[0]) - solved_angles.get(key_list[1]))
                define_variable(var, ans)
                print(var + " = " + str(solved_angles.get(key_list[0])) + " - " + str(solved_angles.get(key_list[1])) + " = " + ans)
    ...
    def finish_side_solve():
        ...
        for var in side_names:
            if var not in solved_sides:
                if var is "c":
                    ans = math.sqrt(solved_sides.get(key_list[0])**2 + solved_sides.get(key_list[1])**2)
                    define_variable(var, ans)
                    print(str(var) + " = sqrt( " + str(solved_sides.get(key_list[0])) + "^2 + " + str(solved_sides.get(key_list[1])) + "^2 ) = " + str(ans))
                else:
                    ans = math.sqrt(abs(solved_sides.get(key_list[0])**2 - solved_sides.get(key_list[1])**2))
                    define_variable(var, ans)
                    print(str(var) + " = sqrt( " + str(solved_sides.get(key_list[0])) + "^2 - " + str(solved_sides.get(key_list[1])) + "^2 ) = " + str(ans))
    ...
    def solve_sides_with_angles():
        ...
        for var in side_names:
            if var not in solved_sides:
                if var is "c":
                    ans = solved_sides.get(key_list[0]) / math.sin(math.radians(globals()[key_list[0].upper()]))
                    define_variable(var, ans)
                    print(str(var) + " = " + str(solved_sides.get(key_list[0])) + " / sin( " + str(math.radians(globals()[key_list[0].upper()])) + " ) = " + str(ans))
                else:
                    ans = globals()["c"] * math.sin(math.radians(globals()[var.upper()]))
                    define_variable(var, ans)
                    print(str(var) + " = " + str(globals()["c"]) + " / sin( " + str(globals()[var.upper()]) + " ) = " + str(ans))
